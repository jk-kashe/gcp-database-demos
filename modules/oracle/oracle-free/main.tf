...
resource "google_compute_instance" "oracle_vm" {
  ...
  metadata = {
    ...
    startup-script = templatefile("${path.module}/templates/startup.sh.tpl", {
      ...
      apex_download_url   = var.apex_download_url,
      oracle_free_tag     = var.oracle_free_tag
    })
...
locals {
  additional_users_for_template = [
    for user in var.additional_db_users :
    {
      username = user.username
      password = random_password.additional_db_user_passwords[user.username].result
      grants   = user.grants
    }
  ]
}
